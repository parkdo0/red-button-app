...
export default function HQGamesPage() {
  ...
  const filtered = useMemo(() => {
    return games.filter((g) => {
      ...
      const matchStatus =
        statusFilter === "ALL" ||
        (statusFilter === "ACTIVE" && g.id > 0) || // isActive 필드가 mock에 없으므로 임시
        (statusFilter === "INACTIVE" && g.id < 0);
      return matchSearch && matchDiff && matchStatus;
    });
  }, [games, search, difficultyFilter, statusFilter]);
        ...
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500">
              마스터 게임 DB · 총 {filtered.length}건
            </p>
          </div>
          <Link
            href="/admin/hq/games/new"
            className="flex items-center gap-1.5 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-red-700"
          >
        ...
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-sm text-gray-900 placeholder-gray-400 focus:border-red-300 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-200"
            />
          </div>
          ...
          <select
            ...
            onChange={(e) => { setDifficultyFilter(e.target.value as Difficulty | "ALL"); setPage(1); }}
            className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 focus:border-red-300 focus:outline-none"
          >
                    ...
                    <tr key={game.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          {/* 썸네일 대체 */}
                          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-gray-100 text-lg">
                            🎲
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-bold ${difficultyColor[game.difficulty]}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <Link
                            href={`/admin/hq/games/${game.id}/edit`}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
              <p className="text-xs text-gray-400">
                {(page - 1) * ITEMS_PER_PAGE + 1}-{Math.min(page * ITEMS_PER_PAGE, filtered.length)} / {filtered.length}건
              </p>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page === 1}
                  className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  ← 이전
                </button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors ${
                      p === page ? "bg-red-600 text-white" : "text-gray-500 hover:bg-gray-100"
                    }`}
                  >
                    {p}
                  </button>
