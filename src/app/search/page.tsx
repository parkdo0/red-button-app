...
import { MOCK_GAMES, FILTER_OPTIONS, DIFFICULTY_LABEL, type Game, type Difficulty } from "@/data/mock";
  ...
  return games.filter((game) => {
    ...
    if (keyword) {
      if (isChosungOnly(keyword)) {
        const titleChosung = getChosung(game.title);
        if (!titleChosung.includes(keyword)) return false;
      } else {
        const match =
    ...
    if (filters.playTime.size > 0) {
      if (!filters.playTime.has(game.playTimeCategory)) return false;
    }
...
/**
 * 게임 검색 페이지 - 실제 레드버튼 앱 기준
 * 풀스크린 필터 + 초성 검색 + 가로형 결과 카드
 */
export default function SearchPage() {
  ...
  // 검색 결과가 0일 때 연관 추천 (필터 일부 완화)
  const suggestedGames = useMemo(() => {
    if (results.length > 0 || !isSearchActive) return [];
    // 장르 기반 추천 또는 전체
    if (filters.genre.size > 0) {
      return MOCK_GAMES.filter((g) =>
        g.tags.some((t) => t.group === "genre" && filters.genre.has(t.value))
      ).slice(0, 7);
    }
    return MOCK_GAMES.slice(0, 7);
  }, [results, isSearchActive, filters.genre]);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        ...
        <div className="sticky top-0 z-10 bg-bg-primary/95 backdrop-blur-sm px-6 pt-5 pb-3 md:px-8">
          <div className="relative">
            <input
              type="text"
              placeholder="게임 이름(초성 가능)으로 검색"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="rb-input w-full py-3 pl-4 pr-12 text-sm"
            />
            {/* 검색 버튼 */}
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className="absolute right-12 top-1/2 -translate-y-1/2 flex h-6 w-6 items-center justify-center rounded-full bg-bg-elevated text-text-muted hover:text-text-primary transition-colors"
              >
        ...
        {!isSearchActive ? (
          ...
          /* ========== 검색 결과 ========== */
          <div className="px-6 pb-8 md:px-8">
            {/* 게임 시간 quick filter (상단 고정) */}
            <div className="flex items-center gap-2 mb-4">
              <span className="text-xs font-semibold text-text-muted mr-1">게임 시간</span>
              {FILTER_OPTIONS.playTime.map((opt) => (
                <button
                  key={opt}
                  onClick={() => toggle("playTime", opt)}
                  className={`
                    rounded-full px-3 py-1.5 text-xs font-medium transition-all touch-feedback
                    ${filters.playTime.has(opt)
                      ? "bg-red-primary text-white"
                      : "bg-bg-card text-text-muted border border-border-default hover:border-border-hover"
                    }
                  `}
                >
                  {opt}
            ...
            <div className="mb-3">
              <span className="text-sm font-bold text-text-primary">
                검색 결과{" "}
                <span className="text-red-primary">{results.length}</span>
              </span>
            </div>
            ...
            {results.length > 0 ? (
              <div className="flex flex-col gap-3">
                {results.map((game, index) => (
                  <GameSearchCard key={game.id} game={game} index={index} />
                ))}
              </div>
            ) : (
              <>
                ...
                {suggestedGames.length > 0 && (
                  <div className="mt-2">
                    <div className="mb-3 flex items-center gap-2">
                      <span className="text-sm font-bold text-text-primary">
                        연관 추천 게임{" "}
                        <span className="text-red-primary">{suggestedGames.length}</span>
                      </span>
                    </div>
                    <div className="flex flex-col gap-3">
            ...
            {hasAnyFilter && (
              <button
                onClick={clearAll}
                className="mt-4 w-full rounded-xl border border-border-default py-2.5 text-sm font-medium text-text-muted hover:text-red-primary hover:border-red-primary/30 transition-colors touch-feedback"
              >
                필터 초기화
              </button>
            )}
      ...
      <button className="fixed right-6 bottom-6 flex h-12 w-12 items-center justify-center rounded-full bg-bg-card border border-border-default shadow-lg touch-feedback hover:border-red-primary/30 transition-colors">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="text-red-primary">
...
function FilterSection({ filters, onToggle, onClear, hasAnyFilter }: FilterSectionProps) {
  return (
    <div className="flex flex-col gap-6">
      ...
      <FilterGroup
        label="인원수"
        options={[...FILTER_OPTIONS.playerCount]}
        selected={filters.playerCount}
        onToggle={(v) => onToggle("playerCount", v)}
      />
      ...
      <FilterGroup
        ...
        renderLabel={(v) => DIFFICULTY_LABEL[v as Difficulty] ?? v}
      />

      {/* 게임 시간 */}
      <FilterGroup
        label="게임 시간"
        options={[...FILTER_OPTIONS.playTime]}
        selected={filters.playTime}
        onToggle={(v) => onToggle("playTime", v)}
      />
      ...
      {hasAnyFilter && (
        <div className="flex gap-3 pt-2">
          <button
            onClick={onClear}
            className="flex-1 rounded-xl border border-border-default py-2.5 text-sm font-medium text-text-muted hover:text-text-primary transition-colors touch-feedback"
          >
            초기화
          </button>
        </div>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`
                rounded-full px-4 py-2 text-[13px] font-medium transition-all duration-150 touch-feedback
                ${isActive
                  ? "bg-red-primary text-white shadow-sm"
                  : "bg-bg-card text-text-secondary border border-border-default hover:border-border-hover hover:text-text-primary"
                }
              `}
            >
