...
export default function OrderPage() {
  ...
  const filteredMenus = getFilteredMenus(activeTab);
...
/** 카테고리별 메뉴 필터 */
function getFilteredMenus(tab: string): MenuItem[] {
  switch (tab) {
    case "NEW":
      return MOCK_MENUS.filter((m) => m.isNew);
    case "BEST":
      return MOCK_MENUS.filter((m) => m.isBest);
    default:
      return MOCK_MENUS.filter((m) => m.categoryName === tab);
  }
}
